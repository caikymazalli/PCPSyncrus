...
app.get('/', (c) => {
          ...
          <tr data-search="${escapeHtml((inst.title || '').toLowerCase())} ${escapeHtml((inst.code || '').toLowerCase())} ${escapeHtml((inst.description || '').toLowerCase())}">
            ...
            <td>
              <div style="display:flex;gap:4px;flex-wrap:wrap;">
                ...
                <button class="btn btn-sm" style="background:#e8f4fd;color:#1B4F72;border:1px solid #bdd7ee;" title="Usar esta instrução como modelo para uma nova" onclick="openUseAsTemplate('${escapeHtml(inst.id)}','${escapeHtml(inst.title || '')}','${escapeHtml(inst.code || '')}')"><i class="fas fa-copy"></i> Usar como Modelo</button>
              </div>
            </td>
          </tr>`).join('')}
        </tbody>
  ...
  <div class="modal-overlay" id="useAsTemplateModal">
    <div class="modal" style="max-width:540px;">
      <div style="padding:20px 24px;border-bottom:1px solid #f1f3f5;display:flex;align-items:center;justify-content:space-between;">
        <h3 style="margin:0;font-size:17px;font-weight:700;color:#1B4F72;"><i class="fas fa-copy" style="margin-right:8px;color:#2980B9;"></i>Usar como Modelo</h3>
        <button onclick="closeModal('useAsTemplateModal')" style="background:none;border:none;font-size:20px;cursor:pointer;color:#9ca3af;">×</button>
      </div>
      <div style="padding:20px 24px;">
        <div style="background:#e8f4fd;border:1px solid #bdd7ee;border-radius:8px;padding:12px 16px;margin-bottom:18px;font-size:13px;color:#1B4F72;">
          <i class="fas fa-info-circle" style="margin-right:6px;"></i>
          Serão copiadas apenas as <strong>etapas (título, descrição e observação)</strong>.<br>
          <span style="color:#6c757d;">Imagens não serão transferidas.</span>
        </div>
        <div style="background:#f8f9fa;border-radius:8px;padding:10px 14px;margin-bottom:18px;font-size:13px;">
          <span style="color:#6c757d;">Modelo de origem:</span>
          <strong id="templateSourceTitle" style="color:#1B4F72;margin-left:6px;">—</strong>
        </div>
        <div class="form-group">
          <label class="form-label">Código da nova instrução</label>
          <input class="form-control" id="tmplNewCode" type="text" placeholder="Ex: INSTR-002">
        </div>
        <div class="form-group">
          <label class="form-label">Título da nova instrução *</label>
          <input class="form-control" id="tmplNewTitle" type="text" placeholder="Ex: Procedimento de Fresagem CNC">
        </div>
        <div class="form-group">
          <label class="form-label">Descrição</label>
          <textarea class="form-control" id="tmplNewDesc" rows="2" placeholder="Breve descrição..."></textarea>
        </div>
      </div>
      <div style="padding:16px 24px;border-top:1px solid #f1f3f5;display:flex;justify-content:flex-end;gap:10px;">
        <button onclick="closeModal('useAsTemplateModal')" class="btn btn-secondary">Cancelar</button>
        <button class="btn btn-primary" id="tmplSaveBtn" onclick="saveFromTemplate()"><i class="fas fa-copy"></i> Criar a partir do modelo</button>
      </div>
    </div>
  </div>
  ...
  let _templateSourceId = null

  function openUseAsTemplate(id, title, code) {
    _templateSourceId = id
    document.getElementById('templateSourceTitle').textContent = (code ? '[' + code + '] ' : '') + title
    document.getElementById('tmplNewCode').value = ''
    document.getElementById('tmplNewTitle').value = ''
    document.getElementById('tmplNewDesc').value = ''
    openModal('useAsTemplateModal')
  }

  async function saveFromTemplate() {
    if (!_templateSourceId) return
    const code = document.getElementById('tmplNewCode').value.trim()
    const title = document.getElementById('tmplNewTitle').value.trim()
    const desc = document.getElementById('tmplNewDesc').value.trim()
    ...
    try {
      const res = await fetch('/instrucoes/api/instructions/' + encodeURIComponent(_templateSourceId) + '/clone-as-template', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code, title, description: desc })
      })
      ...
      if (res.ok && data && data.ok) {
        alert('✅ Instrução criada com sucesso a partir do modelo!')
        closeModal('useAsTemplateModal')
        setTimeout(() => location.reload(), 500)
      } else {
        alert('❌ Erro: ' + ((data && data.error) || 'Desconhecido'))
      }
    } catch (e) {
      alert('❌ Falha de conexão: ' + (e && e.message ? e.message : e))
    } finally {
      btn.disabled = false
      btn.innerHTML = '<i class="fas fa-copy"></i> Criar a partir do modelo'
    }
  }
  </script>
  `
...
app.get('/:id', (c) => {
  ...
  <div class="section-header">
    ...
    <div class="wi-actions no-print">
      ...
      <span id="reorderIndicator" style="display:none;font-size:12px;color:#666;margin-left:4px;"></span>
    </div>
  </div>

  <div class="card no-print" style="padding:10px 20px;margin-bottom:8px;background:#fffbea;border:1px solid #ffe58f;border-radius:8px;display:flex;align-items:center;gap:10px;font-size:13px;color:#7d6608;">
    <i class="fas fa-arrows-alt" style="font-size:16px;"></i>
    <span>Arraste as linhas pela alça <strong><i class="fas fa-grip-vertical"></i></strong> para reordenar as etapas. A nova ordem é salva automaticamente ao soltar.</span>
  </div>

  <div class="card" style="overflow:hidden;">
    <div class="table-wrapper">
      <table class="wi-table" id="stepsTable">
        <thead>
          <tr>
        ...
        <tbody id="stepsBody">
          ${steps.length === 0 ? `
            <tr id="empty-row">
              <td colspan="7" style="padding:16px;color:#6c757d;">Nenhuma etapa cadastrada para esta instrução.</td>
            </tr>
          ` : steps.map((step: any) => {
            const stepPhotos = photosByStep[step.id] || []
            const firstPhoto = stepPhotos[0]
            return `
              <tr data-step-id="${escapeHtml(step.id)}" style="cursor:default;">
                <td class="no-print" style="text-align:center;padding:8px 4px;cursor:grab;">
                  <span class="drag-handle" title="Arrastar para reordenar" style="color:#adb5bd;font-size:16px;user-select:none;cursor:grab;display:inline-block;padding:4px;">
                    <i class="fas fa-grip-vertical"></i>
                  </span>
                </td>
                <td>
                  <input class="form-control" type="number" min="1" step="1" data-field="step_number" value="${escapeHtml(step.step_number)}" style="width:60px;">
                </td>
                <td>
                ...
                <td class="no-print">
                  <div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;">
                    <button class="btn btn-primary btn-sm" onclick="saveStep('${escapeHtml(instructionId)}', '${escapeHtml(step.id)}')"><i class="fas fa-save"></i> Salvar</button>
                    <button class="btn btn-secondary btn-sm" onclick="deleteStep('${escapeHtml(instructionId)}', '${escapeHtml(step.id)}')"><i class="fas fa-trash"></i> Remover</button>
                  </div>
                </td>
              </tr>
            `
  ...
  // ── Reordenação de etapas via drag-and-drop (SortableJS) ──────────────────
  const _instrId = '${escapeHtml(instructionId)}'

  /** Renumera os inputs [data-field="step_number"] na ordem atual das linhas */
  function renumberStepInputs() {
    const rows = document.querySelectorAll('#stepsBody tr[data-step-id]')
    rows.forEach((row, idx) => {
      const inp = row.querySelector('[data-field="step_number"]')
      if (inp) inp.value = idx + 1
    })
  }

  /** Persiste a nova ordem de etapas via API reorder */
  async function persistReorder() {
    ...
    const indicator = document.getElementById('reorderIndicator')
    if (indicator) { indicator.style.display = 'inline'; indicator.textContent = '⏳ Salvando ordem...' }
    try {
      const res = await fetch('/instrucoes/api/instructions/' + encodeURIComponent(_instrId) + '/steps/reorder', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json()
      if (indicator) {
        indicator.textContent = (data && data.ok) ? '✅ Ordem salva' : '❌ Erro ao salvar'
        setTimeout(() => { indicator.style.display = 'none' }, 2500)
      }
    } catch (e) {
      if (indicator) { indicator.textContent = '❌ Falha de conexão'; setTimeout(() => { indicator.style.display = 'none' }, 2500) }
    }
  }

  // Inicializar SortableJS assim que o script carregar
  ;(function initSortable() {
    if (typeof Sortable === 'undefined') {
      // Carregamento dinâmico caso o CDN ainda não tenha carregado
  ...
  <style>
    ...
    .sortable-chosen { background: #f0f9ff !important; box-shadow: 0 2px 8px rgba(41,128,185,0.18); }
    .drag-handle:hover { color: #2980B9 !important; }
    #reorderIndicator { font-size:12px; margin-left:12px; color:#666; }
  </style>
...
// ── API: POST /api/instructions/:id/clone-as-template (Clonar como Modelo) ──
// Cria nova instrução copiando apenas as etapas (texto) — sem imagens.
app.post('/api/instructions/:id/clone-as-template', async (c) => {
  const authErr = await ensureAuthenticatedOr401(c)
  if (authErr) return authErr
  ...
  // Localizar instrução de origem
  const sourceInstr = (tenant.workInstructions || []).find((i: any) => i.id === sourceId)
  if (!sourceInstr) return err(c, 'Instrução de origem não encontrada', 404)

  const sourceVersion = (tenant.workInstructionVersions || []).find(
    (v: any) => v.instruction_id === sourceId && v.is_current
  )

  // Etapas de origem (apenas texto — sem fotos)
  const sourceSteps = (tenant.workInstructionSteps || [])
    .filter((s: any) => s.version_id === sourceVersion?.id)
    .sort((a: any, b: any) => (a.step_number || 0) - (b.step_number || 0))

  const now = new Date().toISOString()
  const newInstrId = genId('instr')
  ...
  if (db && userId !== 'demo-tenant') {
    try {
      await db.prepare(
        `INSERT INTO work_instructions (id, user_id, empresa_id, code, title, version, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(newInstrId, userId, empresaId, newInstr.code, newInstr.title, '1.0', 'draft', now, now).run()
    } catch (e: any) {
      const msg = e?.message || String(e)
      if (isSchemaDriftError(msg)) return schemaDriftError(c, msg)
      return err(c, 'Erro ao salvar instrução no banco de dados', 500)
    }
    try {
      await db.prepare(
        `INSERT INTO work_instruction_versions (id, user_id, empresa_id, instruction_id, version, title, description, is_current, status, created_at, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(newVersionId, userId, empresaId, newInstrId, '1.0', body.title, body.description || '', 1, 'draft', now, userId).run()
    } catch (e: any) {
      // Rollback instrução inserida
      try { await db.prepare('DELETE FROM work_instructions WHERE id = ? AND user_id = ?').bind(newInstrId, userId).run() } catch {}
      const msg = e?.message || String(e)
      if (isSchemaDriftError(msg)) return schemaDriftError(c, msg)
      return err(c, 'Erro ao salvar versão no banco de dados', 500)
    }
  ...
  // Copiar etapas (apenas texto — sem fotos)
  const newSteps: any[] = []
  for (const srcStep of sourceSteps) {
    const newStepId = genId('step')
    const newStep = {
      id: newStepId,
      ...
      observation: srcStep.observation || '',
      created_at: now,
      created_by: userId,
    }

    if (db && userId !== 'demo-tenant') {
      const saved = await dbInsert(db, 'work_instruction_steps', { ...newStep, user_id: userId, empresa_id: empresaId })
      if (!saved) {
        console.warn(`[INSTRUCOES][CLONE] Falha ao inserir etapa ${newStepId} — continuando`)
      }
    }
  ...
  tenant.workInstructions.push(newInstr)
  tenant.workInstructionVersions.push(newVersion)
  tenant.workInstructionSteps.push(...newSteps)
  markTenantModified(userId)

  await logWorkInstructionEvent(db, userId, empresaId, newInstrId, 'CREATED', {
    title: body.title,
    clonedFrom: sourceId,
    stepsCopied: newSteps.length
  }, tenant)

  return ok(c, { instruction: newInstr, version: newVersion, stepsCopied: newSteps.length })
})

// ── API: PUT /api/instructions/:id/steps/reorder (Reordenar Etapas) ──
// Recebe { order: [{ stepId, step_number }] } e persiste a nova numeração.
app.put('/api/instructions/:id/steps/reorder', async (c) => {
  const authErr = await ensureAuthenticatedOr401(c)
  if (authErr) return authErr
  ...
  if (!body || !Array.isArray(body.order) || body.order.length === 0) {
    return err(c, 'order[] é obrigatório')
  }
  ...
  for (const item of body.order as { stepId: string; step_number: number }[]) {
    if (!item.stepId || typeof item.step_number !== 'number') continue

    // Atualizar em memória
    const stepIdx = (tenant.workInstructionSteps || []).findIndex((s: any) => s.id === item.stepId)
    if (stepIdx !== -1) {
      tenant.workInstructionSteps[stepIdx].step_number = item.step_number
    }
    ...
    if (db && userId !== 'demo-tenant') {
      try {
        await db.prepare(
          'UPDATE work_instruction_steps SET step_number = ? WHERE id = ? AND user_id = ?'
        ).bind(item.step_number, item.stepId, userId).run()
      } catch (e: any) {
        errors.push(item.stepId)
        console.warn(`[INSTRUCOES][REORDER] Falha ao atualizar step ${item.stepId}: ${e?.message}`)
      }
    }
  }
  ...
  if (errors.length > 0) {
    return ok(c, { partial: true, errors })
  }
